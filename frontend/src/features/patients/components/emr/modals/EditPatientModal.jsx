...
/**
 * 🛠 EditPatientModal
 * Unified modification portal for established clinical identities with design parity.
 */
export default function EditPatientModal({ isOpen, onClose, onRefresh, patientId, initialData = {} }) {
    ...
    useEffect(() => {
        if (initialData && Object.keys(initialData).length > 0) {
            ...
            setFormData({
                ...
                blood_group: initialData.bloodType || initialData.blood_group || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            addNotification('Identity Shard Updated', 'Clinical record synchronized.', 'success');
            onClose();
            if (onRefresh) onRefresh();
        } catch (err) {
            addNotification('Sync Failure', 'Failed to synchronize clinical shard.', 'error');
        } finally {
    ...
    const sidebar = (
        <React.Fragment>
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-400">Identity Recalibration</p>
            ...
            <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 shadow-xl border border-white/5 space-y-4 animate-in fade-in zoom-in-95 duration-500">
                ...
                <div>
                    <p className="text-[14px] font-black text-slate-900 dark:text-white truncate">
                        {formData.first_name || formData.last_name ? `${formData.first_name} ${formData.last_name}` : 'Subject Shard'}
                    </p>
                    <p className="text-[8px] font-bold text-slate-400 uppercase mt-1 tracking-widest leading-none italic">Recalibrating Shard #{patientId}</p>
                </div>
            </div>
            ...
            <div className="space-y-4 px-2">
                ...
                <div className="flex justify-between items-center border-t border-slate-100 dark:border-white/5 pt-4">
                    ...
                    <span className="text-[10px] font-black text-emerald-500 uppercase italic">Authorized Update</span>
                </div>
            </div>
    ...
    return (
        <Modal
            ...
            subtitle="Recalibrate established clinical identity shard"
            icon={Edit3}
            sidebar={sidebar}
            maxWidth="max-w-2xl"
        >
            <form onSubmit={handleSubmit} className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="p-4 rounded-xl bg-accent-primary/5 border border-accent-primary/10 flex items-center justify-between opacity-80">
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-[9px] font-black uppercase text-slate-500">Authorized Calibration</span>
                   </div>
                ...
                >
                    {isSubmitting ? 'Synchronizing Recalibration...' : 'Commit Modifications'}
                </Button>
            </form>
        </Modal>
    );
