...
export const ShimmerCard = ({ height = '220px', className = '' }) => (
  <div 
    className={`bg-white border border-slate-200 rounded-2xl p-8 relative overflow-hidden animate-pulse ${className}`}
    style={{ height }}
  >
    ...
    <div className="p-8 space-y-6">
      {[1, 2, 3, 4, 5].map(i => (
        <div key={i} className="flex justify-between items-center pb-4 border-b border-slate-50 last:border-0">
          ...
          <div className="w-20 h-5 bg-slate-50 rounded" />
          <div className="w-24 h-5 bg-slate-100 rounded" />
          <div className="w-16 h-8 bg-slate-50 rounded-full" />
        </div>
      ))}
    </div>
  </div>
...
export const ChartSkeleton = () => (
    <div className="bg-white border border-slate-200 rounded-2xl p-6 h-[400px] animate-pulse flex flex-col">
        <div className="flex justify-between items-start mb-8">
            <div className="space-y-2">
                <div className="w-32 h-4 bg-slate-100 rounded" />
                <div className="w-48 h-6 bg-slate-50 rounded" />
            </div>
        ...
        <div className="flex-1 flex flex-col justify-end gap-2">
            <div className="flex items-end gap-4 h-full px-4">
                {[40, 70, 45, 90, 65, 80, 55].map((h, i) => (
                    <div key={i} className="flex-1 bg-slate-50 rounded-t-lg" style={{ height: `${h}%` }} />
                ))}
            </div>
