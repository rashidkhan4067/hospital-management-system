 ...
 */
const AppointmentsTable = ({ appointments = [], isLoading = false, onStatusChange, onViewDetail }) => {
   if (isLoading) return (
                     ...
                     <tr key={idx} className="group/row hover:bg-slate-50 dark:hover:bg-white/5 transition-all duration-300">
                        <td className="px-3 py-6">
                           <div className="flex items-center gap-4">
                              <div className="relative group cursor-pointer" onClick={() => onViewDetail(a)}>
                                 <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-indigo-500/5 flex items-center justify-center text-indigo-500 font-black text-xs uppercase border border-indigo-500/10 italic shrink-0 shadow-lg group-hover:rotate-6 transition-transform">
                        ...
                        <td className="px-3 py-6 text-right">
                           <div className="flex items-center justify-end gap-2.5 opacity-0 group-hover/row:opacity-100 transition-opacity translate-x-2 group-hover/row:translate-x-0 transition-transform">
                              <button onClick={() => onViewDetail(a)} className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-500 hover:text-accent-primary border border-slate-200 dark:border-white/10 transition-all hover:scale-110 shadow-sm"><Eye size={16} /></button>
                              <button onClick={() => onStatusChange(a, 'completed')} className="w-9 h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-500/20 transition-all hover:scale-110 shadow-sm"><CheckCircle size={16} /></button>
                              <button onClick={() => onStatusChange(a, 'cancelled')} className="w-9 h-9 rounded-xl bg-rose-500/10 flex items-center justify-center text-rose-500 border border-rose-500/20 transition-all hover:scale-110 shadow-sm"><XCircle size={16} /></button>
                           </div>
                        </td>
                     </tr>
                  ))}
