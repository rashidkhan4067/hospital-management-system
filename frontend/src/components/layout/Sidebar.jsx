...
import { getNavigationByRole, COMMON_NAV } from '../../constants/navigation';
...
export default function Sidebar() {
  ...
  const { isSidebarCollapsed, toggleSidebarCollapse, isMobileMenuOpen, setIsMobileMenuOpen } = useUI();
  ...
  return (
    <>
      ...
      <motion.aside 
        initial={false}
        animate={{ 
          width: window.innerWidth < 1024 ? 260 : (isSidebarCollapsed ? 80 : 256),
          x: window.innerWidth < 1024 ? (isMobileMenuOpen ? 0 : -260) : 0
        }}
      ...
      >
        <div className="p-6 min-h-[80px] flex items-center justify-between overflow-hidden">
          <div className="sidebar-brand flex items-center gap-3 px-2 text-slate-900 dark:text-white font-extrabold text-2xl tracking-tighter shrink-0">
            ...
            {(!isSidebarCollapsed || window.innerWidth < 1024) && <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }}>Al Shifaa</motion.span>}
          </div>
        ...
        <nav className="sidebar-links flex flex-col gap-1 px-4 py-8 flex-1 overflow-y-auto custom-scrollbar">
          {primary.map((link) => (
             <SidebarLink key={link.to} {...link} isCollapsed={window.innerWidth >= 1024 ? isSidebarCollapsed : false} />
          ))}
        </nav>
        ...
        <div className="sidebar-footer mt-auto p-4 border-t border-slate-200 dark:border-white/5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <button 
              onClick={logout}
              className={`flex items-center rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-500/5 transition-all duration-300 font-medium text-sm ${isSidebarCollapsed && window.innerWidth >= 1024 ? 'justify-center p-3' : 'gap-4 px-6 py-4 mt-2'}`}
            >
              <LogOut size={20} className="shrink-0" />
              {(!isSidebarCollapsed || window.innerWidth < 1024) && <span>Sign Out</span>}
            </button>
          </div>
        </div>
      </motion.aside>
