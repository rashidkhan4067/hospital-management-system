...
import { Badge, Button, PageHeader, StatsCard, Card, TableActions } from '../../../components/ui';
import AdminTable from '../../../components/features/admin/AdminTable';
import FilterBar from '../../../components/features/admin/FilterBar';
import ProvisionSupplyShardModal from '../../../components/modals/admin/logistics/ProvisionSupplyShardModal';
...
export default function AdminInventory() {
  ...
  const handleRegister = async (data, resetForm) => {
    ...
    try {
        ...
        addNotification('Stock Updated', 'Inventory has been updated successfully.', 'success');
        setIsModalOpen(false);
        resetForm();
        refresh();
    } catch (error) {
        addNotification('Error', 'Could not update inventory.', 'error');
    } finally {
    ...
    { 
        ...
        cell: (a) => (
            <div className="flex items-center gap-4 group">
                <div className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-all group-hover:scale-110 group-hover:rotate-3 ${
                ...
                <div className="flex flex-col">
                    ...
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic tabular-nums">{a.sku}</p>
                </div>
            </div>
        )
    },
        ...
        cell: (a) => (
            <TableActions 
                ...
                actions={[
                    ...
                    { label: 'Modify Assets', icon: Edit, onClick: (row) => console.log('Edit', row) },
                    { label: 'Sync Stock', icon: RefreshCw, onClick: (row) => refresh() },
                    { label: 'Terminate Node', icon: Trash, onClick: (row) => console.log('Delete', row), variant: 'danger' },
                ]}
            />
        )
    },
