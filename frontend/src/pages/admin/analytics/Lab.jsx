...
export default function AdminLab() {
  ...
  const handleRequestTest = async (formData, resetForm) => {
    ...
    try {
        ...
        refresh(); // Refresh results matrix
        resetForm(); // Clear modal internal state
    } catch (err) {
    ...
    { 
        ...
        cell: (r) => (
            <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-tight text-text-secondary">{r.status}</span>
            </div>
        )
    },
    { 
